...
Traffic is calculated for the relevant planned decision window, not blindly for the current moment. AMS-to-home uses the current duty-end departure time. Home-to-AMS uses an arrival deadline at the next AMS-starting duty start. Exact home coordinates are stored only in backend local config/database, never committed, and not sent raw to GPT unless a later feature explicitly requires it.
